...
def clean_text(text):
    ...
    for start, end in illegal_chars:
        text = ''.join(c for c in text if not (start <= ord(c) <= end))
    return text
...
def save_progress(date_str):
    ...
    # Git add + commit + push
    try:
        subprocess.run(["git", "add", output_path, backup_path, PROGRESS_FILE], check=True)
        subprocess.run(["git", "commit", "-m", f"Auto-commit: {date_str} 수집 완료"], check=True)
...
while True:
    ...
    for article in articles:
        ...
        date_str = datetime.fromtimestamp(published_on, tz=timezone.utc).strftime('%Y%m%d')
